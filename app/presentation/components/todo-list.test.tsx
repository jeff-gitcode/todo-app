...
describe("TodoList", () => {
  ...
  it("redirects to todo edit page on edit button click", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /Edit/i }).closest("a"),
    ).toHaveAttribute("href", "/presentation/pages/protected/todos/1");
  });
  ...
  it("redirects to todo create page on new button click", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /New/i }).closest("a"),
    ).toHaveAttribute("href", "/presentation/pages/protected/todos/0");
    // await waitFor(() => {
